...
async function currentClaudeConfigDir(): Promise<string> {
  const found = await claudeCodeAdapter.discover(buildContext());
  if (!found.configFound || !found.configPath) {
    throw new Error(
      "未发现 Claude Code 配置目录；请确认 ~/.claude 或 CLAUDE_CONFIG_DIR。"
    );
  }
...
program.action(async () => {
  if (!bareInvocation) {
    const knownCommand = program.commands.some(
...
function parseTrustKind(raw: string): ProviderTrustKind | undefined {
...
trust
  ...
  .action(
    (endpoint: string, opts: { kind: string; reason: string; json?: boolean }) => {
      const kind = parseTrustKind(opts.kind);
      if (!kind) throw new Error("--kind 仅支持 trusted 或 internal。");
...
ignore
  ...
  .action(async (
    taskId: string,
    opts: { rule: string; reason: string; expires?: string; json?: boolean }
  ) => {
    const triaged = triageReport(await scanAll());
    ...
    const candidate = ruleIgnoreCandidatesForTask(task).find(
      (entry) => entry.ruleId === opts.rule
    );
    if (!candidate) {
      console.error(
        `${opts.rule} 不是当前任务中允许项目级忽略的规则；P0/P1、强制修复和高风险家族不能忽略。`
      );
      process.exitCode = 1;
      return;
    }
    ...
    const entry = state.entries.find(
      (item) => item.ruleId === candidate.ruleId && item.agent === candidate.agent
    );
    if (opts.json) {
...
ignore
  ...
  .requiredOption("--reason <reason>", "记录撤销原因")
  .option("--json", "以 JSON 输出")
  .action((ruleId: string, opts: { agent: string; reason: string; json?: boolean }) => {
...
credential
  ...
  .action(async (taskId: string, opts: { json?: boolean }) => {
    const report = triageReport(await scanAll()).activeReport;
    const task = buildActionTasks(buildActionPlan(report)).find(
      (candidate) => candidate.taskId === taskId
    );
...
credential
  ...
  .action(
    async (
      backupId: string,
      opts: { confirm?: string; json?: boolean }
    ) => {
      const input = {
        cwd: process.cwd(),
        backupId,
      ...
      if (!opts.confirm) {
        ...
        if (opts.json) {
          ...
          console.log(
            `恢复预览：将覆盖 ${preview.files} 个 Claude 设置文件；其中 ${preview.changedFiles} 个与备份不同。`
          );
