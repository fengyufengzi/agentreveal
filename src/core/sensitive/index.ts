...
const RULES: SensitiveRule[] = [
  {
    ...
    severity: "high",
    test: (_rel, base) =>
      (base === ".env" || base.startsWith(".env.")) && !EXAMPLE_ENV_RE.test(base),
  },
  {
    id: "private-key",
    label: "私钥或证书文件",
...
function matchRule(relPath: string): SensitiveRule | undefined {
  ...
  const walk = (dir: string, depth: number): void => {
    ...
    for (const entry of entries) {
      ...
      findings.push({
        ...
        recommendation:
          "确认该文件是否必须放在项目目录；如不需要，移出项目或加入 Agent 的 deny/ignore 规则。",
        fixable: false,
      });
    }
  };
