...
import {
  assertClaudePlaintextTask,
  readClaudeCredentialBackupForMigration,
} from "../credential-backup/index.js";
import { atomicWriteFile } from "../fs-safety.js";
import {
  claudeCredentialApiKeyHelper,
...
function readMigrationFiles(configDir: string): MigrationFile[] {
  ...
  return [...allowed]
    ...
    .flatMap((path) => {
      ...
      const plaintextFields = Object.entries(env).filter(
        ([key, value]) =>
          TOKEN_KEYS.has(key.toUpperCase()) &&
          typeof value === "string" &&
          value.trim().length > 0 &&
          !isProxyManagedPlaceholder(value)
      ).length;
      if (plaintextFields === 0) return [];
...
function migrationFingerprint(taskId: string, files: MigrationFile[]): string {
...
export function previewClaudeCredentialMigration(input: {
  ...
  return {
    ...
    files: files.length,
    plaintextFields: files.reduce(
      (total, file) => total + file.plaintextFields,
      0
    ),
...
export function claudePostMigrationVerification(): ClaudePostMigrationVerification {
  return {
    command: "claude auth status --text",
...
export function verifyClaudeCredentialMigrationState(input: {
  taskId: string;
  configPaths: string[];
}): { files: number; apiKeyHelperConfigured: true } {
  const expectedHelper = claudeCredentialApiKeyHelper(input.taskId);
  if (
    input.configPaths.length < 1 ||
  ...
  for (const path of input.configPaths) {
    ...
    if (
      ...
      throw new Error(
        "Claude 配置已不再符合迁移完成状态，请先复扫或恢复，暂不删除备份。"
      );
    }
  }
  return {
...
function transformedContent(
  ...
  if ("env" in parsed) parsed.env = env;
 ...
 */
export function applyClaudeCredentialMigration(
  input: {
    cwd: string;
    task: ActionTask | undefined;
    taskId: string;
...
): ClaudeCredentialMigrationApplyResult {
  assertFingerprint(input.expectedFingerprint);
  assertClaudePlaintextTask(input.task, input.taskId);
  const files = readMigrationFiles(input.configDir);
  if (
    migrationFingerprint(input.taskId, files) !== input.expectedFingerprint
  ) {
  ...
  if (
    backup.files.length !== files.length ||
  ...
  try {
    for (const file of files) {
      ...
      const verified = JSON.parse(readFileSync(file.path, "utf8")) as Record<
        string,
        unknown
      >;
      const verifiedEnv = asRecord(verified.env);
      if (
        Object.keys(verifiedEnv).some((key) => TOKEN_KEYS.has(key.toUpperCase())) ||
