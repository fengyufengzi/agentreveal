import type {
  EffectiveAgentState,
  EffectivePermission,
} from "../posture/types.js";
import type { PostureReport } from "../posture/report.js";

const CONFIDENCE_LABEL: Record<EffectiveAgentState["confidence"], string> = {
  confirmed: "已确认",
  inferred: "推断",
  incomplete: "证据不完整",
};

const AUTH_LABEL: Record<EffectiveAgentState["auth"]["method"], string> = {
...
function permissionLabel(permission: EffectivePermission): string {
...
export function formatPosture(report: PostureReport): string {
  ...
  for (const item of report.agents) {
    ...
    lines.push(
      "",
      `▍${state.displayName}  [${CONFIDENCE_LABEL[state.confidence]}]`,
      `  Provider：${state.route.providerClass ?? "未确认"} · 模型：${state.route.model ?? "未确认"}`,
      `  路由：${state.route.effectiveEndpoint ?? "未确认"} · 代理：${state.route.proxyKind}` +
        `${state.route.realUpstream ? ` · 真实上游：${state.route.realUpstream}` : ""}`,
      `  认证：${AUTH_LABEL[state.auth.method]} · 状态：${state.auth.status}` +
        `${state.auth.sourceKind ? ` · 来源：${state.auth.sourceKind}` : ""}`,
      `  权限：${state.permissions.map(permissionLabel).join(" · ") || "没有可用摘要"}`,
      `  集成：${state.integrations.filter((entry) => entry.enabled).length} 个已启用`
    );
    lines.push("  配置来源：");
    for (const source of state.configSources) {
      lines.push(
        `    - ${source.kind}/${source.scope} [${source.status}]` +
    ...
    for (const plan of item.remediationPlans) {
      lines.push(
        `  处置计划：${plan.title} [${plan.status}]`,
        `    当前：${plan.currentExplanation}`,
        `    目标：${plan.targetState}`
      );
