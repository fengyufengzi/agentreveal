/** scan 命令的终端输出格式化（纯字符串，便于测试/复用）。 */
import type { RiskFinding, RiskLevel } from "../../adapters/types.js";
...
/** 严重度标签（终端友好，无颜色依赖）。 */
const SEVERITY_TAG: Record<RiskLevel, string> = {
  critical: "[严重]",
  high: "[高危]",
  medium: "[中危]",
  low: "[低危]",
...
function pushFindingLines(lines: string[], f: RiskFinding): void {
  lines.push(`  ${SEVERITY_TAG[f.severity]} ${f.title}  (${f.id})`);
  if (f.description) lines.push(`        ${f.description}`);
  if (f.evidence) lines.push(`        证据: ${formatEvidence(f.evidence)}`);
...
/** 单个 Agent 段落。 */
function formatAgentSection(r: AgentScanResult): string[] {
  const lines: string[] = [];
...
function countBySeverity(report: ScanReport): Record<RiskLevel, number> {
...
function formatCorrelationSection(report: ScanReport): string[] {
  if (report.correlations.length === 0) return [];
  const lines: string[] = [];
  lines.push("▍跨 Agent 关联");
  const sorted = [...report.correlations].sort(
    (a, b) => SEVERITY_ORDER[a.severity] - SEVERITY_ORDER[b.severity]
  );
  for (const f of sorted) {
    pushFindingLines(lines, f);
...
export function formatScan(report: ScanReport): string {
  ...
  for (const r of report.results) {
    for (const line of formatAgentSection(r)) lines.push(line);
    lines.push("");
  }

  for (const line of formatCorrelationSection(report)) lines.push(line);
