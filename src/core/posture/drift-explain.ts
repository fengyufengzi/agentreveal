...
const CARD_LABEL: Record<DriftCardClass, string> = {
  "new-coverage": "新增覆盖",
  "lost-coverage": "失去覆盖",
...
const CARD_GUIDANCE: Record<DriftCardClass, string> = {
...
export function classifyDriftEvent(event: DriftEvent): DriftCardClass {
  const k = event.kind;
  const c = event.change;
  const sev = event.severity;

  if (k === "agent-added" || k === "agent-removed") return "agent-lifecycle";
  if (k === "agent-version-changed") return "agent-version";
  if (k === "acceptance-expired" || k === "ignore-expired") return "policy-expired";

  if (k === "risk-reappeared") return "regression";
  if (k === "risk-added") return "new-coverage";
  if (k === "risk-resolved") return "lost-coverage";

  if (k === "auth-source-changed") return "conflict";
  if (k === "provider-route-changed" && sev === "high") return "conflict";
  if (k === "config-source-changed" && isConflictingSource(event)) return "conflict";

  if (k === "permission-changed") {
    if (sev === "high") return "expansion";
    if (sev === "low") return "contraction";
    return "changed-coverage";
  }

  if (k === "integration-added") return "new-coverage";
  if (k === "integration-removed") return "lost-coverage";
  if (k === "integration-changed") return "changed-coverage";

  return classifyByChange(c);
}

function classifyByChange(change: DriftChangeKind): DriftCardClass {
  switch (change) {
 ...
 */
function isConflictingSource(event: DriftEvent): boolean {
  if (!event.previousCategory) return false;
  return /conflicting|conflict/i.test(event.previousCategory);
}

export function cardLabel(cls: DriftCardClass): string {
  return CARD_LABEL[cls];
}

export function cardGuidance(cls: DriftCardClass): string {
...
export function buildDriftCard(event: DriftEvent): DriftCard {
 ...
 * 渲染层会 HTML 转义。
 */
export function previousVsCurrent(event: DriftEvent): string | undefined {
  if (!event.previousCategory) return undefined;
 ...
 */
export function sortByCardPriority(
  events: readonly DriftEvent[]
): Array<{ event: DriftEvent; card: DriftCard }> {
  const PRIORITY_RANK: Record<DriftCardClass, number> = {
    conflict: 0,
    regression: 1,
    expansion: 2,
    "policy-expired": 3,
  ...
  const PRI_RANK = { P0: 0, P1: 1, P2: 2, P3: 3 } as const;
  return events
