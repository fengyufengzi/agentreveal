...
import {
  ...
  AUTH_STATUSES,
  CONFIG_SOURCE_KINDS,
  CONFIG_SOURCE_SCOPES,
  CONFIG_SOURCE_STATUSES,
  EFFECTIVE_CONFIDENCE,
  DRIFT_POLICY_KINDS,
  DRIFT_POLICY_STATUSES,
  INTEGRATION_KINDS,
  PERMISSION_CAPABILITIES,
  PERMISSION_DECISIONS,
  PERMISSION_SCOPES,
  PROXY_KINDS,
  type DriftAgentSnapshot,
  type BaselineMutationResult,
  type BaselinePreview,
  type DriftComparison,
  type DriftPolicySnapshot,
  type DriftPolicyState,
  type DriftSnapshot,
...
const DRIFT_SNAPSHOT_SCHEMA_VERSION = 1 as const;
const AGENT_IDS = [
  "claude-code",
  "codex",
  "cc-switch",
...
const POLICY_AGENT_IDS = [...AGENT_IDS, "workspace"] as const;
...
interface DriftSnapshotDocument {
  schemaVersion: typeof DRIFT_SNAPSHOT_SCHEMA_VERSION;
  scopes: Record<string, DriftSnapshot>;
  observations: Record<string, DriftSnapshot>;
  seenEventIds: Record<string, string[]>;
  audit: PostureAuditEntry[];
}

interface PostureAuditEntry {
  at: string;
...
function snapshotAgent(
  ...
  const routeModelIdentity = optionalIdentity(
    key,
    `model:${state.agentId}`,
    state.route.model
  );
  const effectiveEndpointIdentity = optionalIdentity(
    key,
    `effective-endpoint:${state.agentId}`,
    state.route.effectiveEndpoint
  );
  const realUpstreamIdentity = optionalIdentity(
    key,
    `real-upstream:${state.agentId}`,
  ...
  return {
    ...
    configSources: state.configSources
      .map((source) => {
        const pathIdentity = optionalIdentity(
          key,
          `config-source-path:${state.agentId}:${source.kind}:${source.scope}`,
          source.path
        );
        return {
          kind: source.kind,
      ...
      .sort((left, right) =>
        `${left.kind}\0${left.scope}\0${left.status}\0${left.pathIdentity ?? ""}`.localeCompare(
          `${right.kind}\0${right.scope}\0${right.status}\0${right.pathIdentity ?? ""}`
        )
      ),
    route: {
    ...
    permissions: state.permissions
      ...
      .sort((left, right) =>
        `${left.capability}\0${left.scope}\0${left.decision}\0${left.sourceKind ?? ""}`.localeCompare(
          `${right.capability}\0${right.scope}\0${right.decision}\0${right.sourceKind ?? ""}`
        )
      ),
    integrations: state.integrations
      .map((integration) => {
        const versionIdentity = optionalIdentity(
          key,
          `integration-version:${state.agentId}:${integration.kind}`,
          integration.version
        );
        const sourcePathIdentity = optionalIdentity(
          key,
          `integration-source:${state.agentId}:${integration.kind}`,
          integration.sourcePath
        );
        return {
          kind: integration.kind,
          identity: postureHmacIdentity(
            key,
            `integration:${state.agentId}:${integration.kind}`,
      ...
      .sort((left, right) =>
        `${left.kind}\0${left.identity}`.localeCompare(
...
function snapshotPolicy(
  ...
  for (const ruleId of policy.ruleIds) {
    if (!RULE_IDS.includes(ruleId)) throw new Error("漂移策略规则无效。");
  }
  return {
    kind: policy.kind,
    agentId: policy.agentId,
    subjectIdentity: postureHmacIdentity(
      key,
      `policy:${policy.kind}:${policy.agentId}`,
      policy.subject
    ),
...
function assertIdentifierArray(
  ...
  if (
    value.some(
      (entry) =>
        entry.length > 128 || !/^[A-Za-z_$][A-Za-z0-9_$.-]*$/.test(entry)
    )
  ) {
...
function assertKeys(
  ...
  if (unknown.length > 0) throw new Error(`${label}包含未知字段。`);
}
...
function assertEffectiveState(value: unknown): asserts value is EffectiveAgentState {
  ...
  if (!Array.isArray(value.configSources)) throw new Error("配置来源无效。");
  for (const rawSource of value.configSources) {
    ...
    if (rawSource.path !== undefined) assertString(rawSource.path, "配置来源路径");
  ...
  for (const field of ["model", "effectiveEndpoint", "realUpstream"] as const) {
    if (value.route[field] !== undefined) assertString(value.route[field], "有效路由字段");
  }
  ...
  if (!Array.isArray(value.auth.conflicts)) throw new Error("认证冲突无效。");
  for (const rawConflict of value.auth.conflicts) {
    ...
    if (!Array.isArray(rawConflict.sourceKinds)) throw new Error("认证冲突来源无效。");
    for (const kind of rawConflict.sourceKinds) {
      assertEnum(kind, CONFIG_SOURCE_KINDS, "认证冲突来源");
    }
  }

  if (!Array.isArray(value.permissions)) throw new Error("权限状态无效。");
  for (const rawPermission of value.permissions) {
    assertObject(rawPermission, "权限状态");
    assertKeys(
      rawPermission,
      ["capability", "decision", "scope", "sourceKind"],
      "权限状态"
    );
  ...
  if (!Array.isArray(value.integrations)) throw new Error("集成状态无效。");
  for (const rawIntegration of value.integrations) {
    assertObject(rawIntegration, "集成状态");
    assertKeys(
      rawIntegration,
      ["kind", "identity", "enabled", "version", "sourcePath"],
      "集成状态"
    );
    assertEnum(rawIntegration.kind, INTEGRATION_KINDS, "集成类型");
    assertString(rawIntegration.identity, "集成身份");
    if (typeof rawIntegration.enabled !== "boolean") throw new Error("集成启用状态无效。");
    if (rawIntegration.version !== undefined) assertString(rawIntegration.version, "集成版本");
...
export function buildDriftSnapshot(
  ...
  key: Buffer,
  capturedAt = new Date(),
  policyStates: readonly DriftPolicyState[] = []
): DriftSnapshot {
  if (!Number.isFinite(capturedAt.getTime())) throw new Error("可信快照时间无效。");
...
function emptyDocument(): DriftSnapshotDocument {
  return {
    schemaVersion: DRIFT_SNAPSHOT_SCHEMA_VERSION,
    scopes: {},
    observations: {},
...
function isHmacIdentity(value: unknown): value is HmacIdentity {
  return typeof value === "string" && /^hmac-sha256:[a-f0-9]{64}$/.test(value);
}

function validateDocument(value: unknown): DriftSnapshotDocument {
  assertObject(value, "可信快照文件");
  assertKeys(
    value,
    ["schemaVersion", "scopes", "observations", "seenEventIds", "audit"],
    "可信快照文件"
  );
  ...
  const observations =
    value.observations === undefined ? {} : value.observations;
  ...
  for (const [scopeId, rawSnapshot] of [
    ...
    assertKeys(
      rawSnapshot,
      ["schemaVersion", "capturedAt", "agents", "policies"],
      "可信快照"
    );
    ...
    if (!Array.isArray(rawSnapshot.agents)) throw new Error("可信快照 Agent 列表无效。");
    ...
    for (const rawAgent of rawSnapshot.agents) {
      ...
      if (agentIds.has(rawAgent.agentId)) throw new Error("可信快照包含重复 Agent。");
      ...
      if (!Array.isArray(rawAgent.configSources)) throw new Error("可信快照配置来源无效。");
      for (const rawSource of rawAgent.configSources) {
        assertObject(rawSource, "可信快照配置来源");
        assertKeys(
          rawSource,
          ["kind", "scope", "status", "fieldNames", "pathIdentity"],
          "可信快照配置来源"
        );
      ...
      assertKeys(
        rawAgent.auth,
        ["method", "sourceKind", "status", "conflictCodes"],
        "可信快照认证"
      );
      ...
      if (!Array.isArray(rawAgent.permissions)) throw new Error("可信快照权限无效。");
      ...
      if (!Array.isArray(rawAgent.integrations)) throw new Error("可信快照集成无效。");
      for (const rawIntegration of rawAgent.integrations) {
        assertObject(rawIntegration, "可信快照集成");
        assertKeys(
          rawIntegration,
          ["kind", "identity", "enabled", "versionIdentity", "sourcePathIdentity"],
          "可信快照集成"
        );
        assertEnum(rawIntegration.kind, INTEGRATION_KINDS, "可信快照集成类型");
        if (!isHmacIdentity(rawIntegration.identity)) throw new Error("可信快照集成身份无效。");
        if (typeof rawIntegration.enabled !== "boolean") {
          throw new Error("可信快照集成启用状态无效。");
        }
        for (const field of ["versionIdentity", "sourcePathIdentity"] as const) {
          if (
            rawIntegration[field] !== undefined &&
    ...
    if (!Array.isArray(policies)) throw new Error("可信快照策略列表无效。");
    ...
    for (const rawPolicy of policies) {
      ...
      assertEnum(rawPolicy.priority, ["P0", "P1", "P2", "P3"] as const, "可信快照策略优先级");
      assertEnum(
        rawPolicy.severity,
      ...
      if (policyIds.has(policyId)) throw new Error("可信快照包含重复策略。");
      policyIds.add(policyId);
    }
  }

  const seenEventIds =
    value.seenEventIds === undefined ? {} : value.seenEventIds;
  ...
  for (const rawEntry of audit) {
    ...
    assertEnum(
      rawEntry.action,
      ["create", "replace", "remove"] as const,
      "可信快照审计操作"
    );
    if (
      typeof rawEntry.scopeId !== "string" ||
  ...
  return {
    schemaVersion: DRIFT_SNAPSHOT_SCHEMA_VERSION,
    scopes: value.scopes as Record<string, DriftSnapshot>,
    observations: observations as Record<string, DriftSnapshot>,
    seenEventIds: seenEventIds as Record<string, string[]>,
    audit: audit as PostureAuditEntry[],
  };
}
...
export class PostureSnapshotStore {
  ...
  private readonly policyStates: (now: Date) => readonly DriftPolicyState[];
  ...
  private read(): DriftSnapshotDocument {
  ...
  private write(document: DriftSnapshotDocument): void {
  ...
  private snapshotFingerprint(snapshot: DriftSnapshot): string {
    return `sha256:${createHash("sha256")
      .update(
        JSON.stringify({
  ...
  private buildSnapshot(
    ...
    return buildDriftSnapshot(
      states,
      key,
      capturedAt,
      this.policyStates(capturedAt)
    );
  }
  ...
  saveBaselineConfirmed(
    states: readonly EffectiveAgentState[],
    preview: Pick<
      BaselinePreview,
      "currentFingerprint" | "storageRevision"
    >
  ...
  removeBaseline(options: { expectedStorageRevision?: string } = {}): boolean {
    return this.withMutationLock(() => {
      const revision = this.storageRevision();
      if (
        options.expectedStorageRevision &&
      ...
      const document = this.read();
      if (!document.scopes[this.scopeId]) return false;
