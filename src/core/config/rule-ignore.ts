...
function normalizeReason(reason: string): string {
  ...
  if (!normalized) throw new Error("必须填写项目忽略原因。");
  if (normalized.length > 500) throw new Error("项目忽略原因不能超过 500 个字符。");
  return normalized;
}
...
function parseAgent(value: unknown, label = "agent"): AgentId {
...
function normalizeDate(value: string | undefined, label: string): string | undefined {
  ...
  if (!Number.isFinite(date.getTime())) throw new Error(`${label} 必须是有效日期。`);
...
function parseEntry(value: unknown, index: number): RuleIgnoreEntry {
  const item = asObject(value, `ruleIgnores[${index}]`);
  const reason = normalizeReason(item.reason as string);
  const createdAt = normalizeDate(item.createdAt as string, `ruleIgnores[${index}].createdAt`);
  if (!createdAt) throw new Error(`ruleIgnores[${index}].createdAt 不能为空。`);
  const expiresAt = normalizeDate(
    item.expiresAt as string | undefined,
    `ruleIgnores[${index}].expiresAt`
  );
  return {
...
function readEntries(document: Record<string, unknown>): RuleIgnoreEntry[] {
  const raw = document.ruleIgnores;
  if (raw === undefined) return [];
  if (!Array.isArray(raw)) throw new Error("ruleIgnores 必须是数组。");
...
function parseEvent(value: unknown, index: number): RuleIgnoreEvent {
  ...
  const at = normalizeDate(item.at as string, `ruleIgnoreAudit[${index}].at`);
  if (!at) throw new Error(`ruleIgnoreAudit[${index}].at 不能为空。`);
  const expiresAt = normalizeDate(
    item.expiresAt as string | undefined,
    `ruleIgnoreAudit[${index}].expiresAt`
  );
  return {
    action: item.action,
    ruleId: parseRuleId(item.ruleId, `ruleIgnoreAudit[${index}].ruleId`),
    agent: parseAgent(item.agent, `ruleIgnoreAudit[${index}].agent`),
    reason: normalizeReason(item.reason as string),
    at,
    ...(expiresAt ? { expiresAt } : {}),
  };
}

function readAudit(document: Record<string, unknown>): RuleIgnoreEvent[] {
  const raw = document.ruleIgnoreAudit;
  if (raw === undefined) return [];
  if (!Array.isArray(raw)) throw new Error("ruleIgnoreAudit 必须是数组。");
  return raw.map(parseEvent);
}

function writeState(
  source: ReturnType<typeof readDocument>,
  entries: RuleIgnoreEntry[],
  audit: RuleIgnoreEvent[]
): void {
  const document = {
    ...source.document,
...
function statusOf(entry: RuleIgnoreEntry, now: Date): ListedRuleIgnore["status"] {
...
export function ruleIgnoreEligibility(ruleId: string): {
  allowed: boolean;
  reason?: string;
} {
  const action = getRuleAction(ruleId);
  if (!action) return { allowed: false, reason: "未知规则不能加入项目忽略。" };
...
export function ruleIgnoreCandidatesForTask(task: ActionTask): RuleIgnoreCandidate[] {
  if (!task.agent) return [];
  return task.requirements.flatMap((requirement) => {
    const eligibility = ruleIgnoreEligibility(requirement.ruleId);
    return eligibility.allowed
      ? [{ ruleId: requirement.ruleId as RuleId, agent: task.agent as AgentId }]
      : [];
  });
}

export function listRuleIgnores(cwd: string, now = new Date()): RuleIgnoreState {
...
export function activeRuleIgnores(cwd: string, now = new Date()): ListedRuleIgnore[] {
 ...
 */
export function activeRuleIgnoresSafely(cwd: string, now = new Date()): ListedRuleIgnore[] {
  try {
...
export function addRuleIgnore(input: {
  ...
  const existing = entries.find(
    (entry) => entry.ruleId === ruleId && entry.agent === agent
  );
...
export function removeRuleIgnore(input: {
  ...
  writeState(
    source,
    entries.filter((entry) => entry.ruleId !== ruleId || entry.agent !== agent),
    [...audit, { action: "remove", ruleId, agent, reason, at }]
  );
  return listRuleIgnores(input.cwd, now);
}
