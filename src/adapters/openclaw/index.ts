...
export const openclawAdapter: Adapter = {
  ...
  async discover(ctx: DiscoveryContext): Promise<AgentDiscovery> {
    ...
    if (serviceEnvPresent) notes.push("检测到 service-env/ 目录（凭证文件存在，未读取内容）");
    if (cronPresent) notes.push("检测到 cron/ 目录");
...
function serviceEnvDirPresent(p: string): boolean {
  try {
