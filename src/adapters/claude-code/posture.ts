...
function readJson(path: string): { data?: JsonObject; unreadable?: boolean } {
...
function flattenRelevant(data: JsonObject): string[] {
  const fields: string[] = [];
  ...
  for (const key of [
    ...
    if (data[key] !== undefined) fields.push(key);
  }
  ...
  for (const key of [
    ...
    if (permissions[key] !== undefined) fields.push(`permissions.${key}`);
  }
  const sandbox = asRecord(data.sandbox);
  if (sandbox.enabled !== undefined) fields.push("sandbox.enabled");
  ...
  fields.push(
    ...(
      Object.keys(asRecord(data.enabledPlugins)).length > 0
...
function mergeSettings(target: JsonObject, next: JsonObject): JsonObject {
  const output: JsonObject = { ...target, ...next };
  ...
  for (const key of ["allow", "ask", "deny", "additionalDirectories"]) {
    ...
    if (combined.length > 0) permissions[key] = [...new Set(combined)];
  }
...
function addFileLayer(
  ...
  scope: "machine" | "user" | "project" | "session"
): void {
  const parsed = readJson(path);
  if (!parsed.data && !parsed.unreadable) return;
...
function winningKind(
  layers: readonly Layer[],
  field: string
): ConfigSourceKind | undefined {
...
function permissionSummary(
  settings: JsonObject,
  sourceKind: ConfigSourceKind | undefined
): EffectivePermission[] {
  const mode =
    nonEmpty(asRecord(settings.permissions).defaultMode) ?? "default";
  let filesystemWrite: EffectivePermission["decision"] = "ask";
  let commandExecute: EffectivePermission["decision"] = "ask";
  let outsideWrite: EffectivePermission["decision"] = "ask";
  ...
  const allowedDomains = Array.isArray(sandboxNetwork.allowedDomains)
    ...
    : [];
  const networkDecision: EffectivePermission["decision"] =
...
function mcpIntegrations(
  home: string,
  cwd: string
): { integrations: EffectiveIntegration[]; sources: SourceContribution[]; unreadable: boolean } {
  ...
  if (global.unreadable) unreadable = true;
  if (global.data) {
    candidates.push({
      kind: "user",
      path: globalPath,
  ...
  if (project.unreadable) unreadable = true;
  if (project.data) {
    candidates.push({
      kind: "project-shared",
      path: projectPath,
  ...
  if (global.data) {
    ...
    const localEntry = Object.entries(projects).find(
      ([path]) => resolve(path) === canonical
    );
    if (localEntry) {
      candidates.push({
        kind: "project-local",
  ...
  const byName = new Map<
    string,
    { raw: JsonObject; kind: ConfigSourceKind; path: string }
  >();
...
export function buildClaudeEffectiveState(
  input: ClaudePostureInput
): EffectiveAgentState {
  const layers: Layer[] = [];
  addFileLayer(
    layers,
    join(input.configDir, "settings.json"),
    "user",
    "user"
  );
  addFileLayer(
  ...
  const settings = layers.reduce<JsonObject>(
    (current, layer) => mergeSettings(current, layer.data),
    {}
  );
  const settingsEnv = asRecord(settings.env);
  const effectiveEnv: Record<string, unknown> = { ...settingsEnv };
  for (const [key, value] of Object.entries(input.env)) {
    if (value !== undefined) effectiveEnv[key] = value;
  }
  const envSource = (key: string): ConfigSourceKind =>
    input.env[key] !== undefined
      ? "environment"
      : winningKind(layers, `env.${key}`) ?? "environment";

  const authCandidates: Array<{
    method: EffectiveAgentState["auth"]["method"];
    sourceKind: ConfigSourceKind;
    code: string;
  }> = [];
  if (
    enabledEnvironmentFlag(effectiveEnv.CLAUDE_CODE_USE_BEDROCK) ||
  ...
  if (
    typeof effectiveEnv.CLAUDE_CODE_OAUTH_TOKEN === "string" &&
  ...
  for (const [name, enabled] of Object.entries(
    asRecord(settings.enabledPlugins)
  )) {
    integrations.push({
      kind: "skill",
      identity: name,
  ...
  return {
    ...
    confidence:
      layers.some((layer) => layer.contribution.unreadable) ||
      mcp.unreadable ||
    ...
    route: {
      ...
      model:
        nonEmpty(effectiveEnv.ANTHROPIC_MODEL) ??
    ...
    permissions: permissionSummary(
      settings,
      winningKind(layers, "permissions.defaultMode")
    ),
    integrations,
