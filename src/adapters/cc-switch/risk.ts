...
function activeProviders(data: CcSwitchData): CcProvider[] {
  return data.providers.filter((p) => p.isCurrent || p.inFailoverQueue);
}

export function buildCcSwitchFindings(
  data: CcSwitchData,
  ...
  const byFp = new Map<string, CcProvider[]>();
  for (const p of data.providers) {
    if (!p.keyFingerprint) continue;
  ...
  for (const proxy of data.proxies) {
    ...
    if (proxy.autoFailover) {
      const unknowns = data.providers.filter(
        (p) =>
          p.appType === proxy.appType &&
          p.inFailoverQueue &&
          p.baseUrl &&
          ["openai_compatible_unknown", "unknown", "relay_or_proxy"].includes(
            classifyBaseUrl(p.baseUrl, providerPolicy).type
          )
      );
