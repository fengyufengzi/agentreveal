...
function readToml(path: string): { data?: TomlObject; unreadable?: boolean } {
...
function deepMerge(base: TomlObject, next: TomlObject): TomlObject {
...
function addLayer(
  ...
  options: { ignored?: boolean; data?: TomlObject; unreadable?: boolean } = {}
): void {
  const parsed =
...
function projectRoot(cwd: string, markers: readonly string[] = [".git"]): string {
...
function trustForProject(user: TomlObject, cwd: string): boolean {
  const canonical = resolve(cwd);
  const markers = Array.isArray(user.project_root_markers)
    ? user.project_root_markers.filter(
  ...
  for (const [path, value] of Object.entries(asRecord(user.projects))) {
    if (
      [canonical, root].includes(resolve(path)) &&
...
function sanitizeProjectLayer(data: TomlObject): TomlObject {
  return Object.fromEntries(
    Object.entries(data).filter(([key]) => !PROJECT_IGNORED_ROOT_KEYS.has(key))
  );
}
...
function permissionSummary(
  ...
  if (defaultPermissions === ":read-only") sandboxMode = "read-only";
  if (defaultPermissions === ":workspace") sandboxMode = "workspace-write";
  if (defaultPermissions === ":danger-full-access") {
    sandboxMode = "danger-full-access";
  }
  const approval = settings.approval_policy;
  const commandDecision: EffectivePermission["decision"] =
    approval === "never"
      ? "allow"
...
function sourceKindForField(
  layers: readonly Layer[],
  field: string
): ConfigSourceKind | undefined {
  for (let index = layers.length - 1; index >= 0; index -= 1) {
    if (
      !layers[index].ignored &&
...
export function buildCodexEffectiveState(
  input: CodexPostureInput
): EffectiveAgentState {
  const layers: Layer[] = [];
  ...
  const trusted =
    input.projectTrusted ?? trustForProject(user, input.cwd);
  ...
  for (const path of projectConfigPaths(input.cwd, rootMarkers)) {
    ...
    if (!parsed.data && !parsed.unreadable) continue;
  ...
  const winningContributions = layers
    ...
    .map((layer) => {
      ...
      const fields =
        contribution.source.scope === "project"
          ? contribution.fields.filter(
              (field) => !PROJECT_IGNORED_ROOT_KEYS.has(field.split(".")[0])
            )
          : contribution.fields;
  ...
  let effective: TomlObject = {};
  for (const layer of layers) {
    if (layer.ignored || layer.contribution.unreadable) continue;
    const data =
      layer.contribution.source.scope === "project"
  ...
  const integrations: EffectiveIntegration[] = Object.entries(mcp).map(
    ([name, raw]) => {
  ...
  const permissionSource =
    sourceKindForField(layers, "default_permissions") ??
  ...
  return {
    ...
    route: {
      ...
      proxyKind:
        providerClass === "local" || authFile.proxyManaged
          ? "custom"
          : "none",
