...
function permissionMap(snapshot) {
    return new Map(snapshot.permissions.map((permission) => [
        permission.capability,
        permission,
    ]));
}
function compareAgent(baseline, current) {
    const events = [];
    ...
    if (stable(baseline.route) !== stable(current.route)) {
        events.push(event({
            ...
            previousCategory: baseline.route.providerClass ?? baseline.route.proxyKind,
    ...
    const baselineIntegrations = new Map(baseline.integrations.map((integration) => [
        `${integration.kind}:${integration.identity}`,
        integration,
    ]));
    const currentIntegrations = new Map(current.integrations.map((integration) => [
        `${integration.kind}:${integration.identity}`,
        integration,
    ]));
    ...
    for (const [identity, integration] of baselineIntegrations) {
        if (currentIntegrations.has(identity))
            continue;
        events.push(event({
            agentId,
...
function policyMap(snapshot) {
    return new Map((snapshot.policies ?? []).map((policy) => [
        `${policy.kind}\0${policy.agentId}\0${policy.subjectIdentity}`,
        policy,
    ]));
}
function expiredPolicyEvents(baseline, current) {
    ...
    return [...policyMap(current).entries()].flatMap(([identity, policy]) => {
        ...
        if (previous?.status !== "active" || policy.status !== "expired")
            return [];
...
function activeEvents(baseline, current) {
    const events = [];
    const baselineAgents = new Map(baseline.agents.map((agent) => [agent.agentId, agent]));
    const currentAgents = new Map(current.agents.map((agent) => [agent.agentId, agent]));
...
function resolvedEvent(previous) {
    return {
        ...previous,
        change: "removed",
        priority: "P3",
...
function sortEvents(events) {
    return events.sort((left, right) => PRIORITY_ORDER[left.priority] - PRIORITY_ORDER[right.priority] ||
        SEVERITY_ORDER[left.severity] - SEVERITY_ORDER[right.severity] ||
        left.agentId.localeCompare(right.agentId) ||
        left.eventId.localeCompare(right.eventId));
}
export function compareDriftSnapshots(baseline, current, options = {}) {
    if (!baseline) {
        return {
            comparison: {
                status: "no-baseline",
    ...
    for (const entry of currentEvents) {
        if (!previousIds.has(entry.eventId) && seenIds.has(entry.eventId)) {
            ...
            if (entry.kind === "risk-added")
                entry.kind = "risk-reappeared";
