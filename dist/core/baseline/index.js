 ...
 */
export function baselinePlanFingerprint(plan) {
    const payload = {
...
function renderChangeDiff(path, changes) {
    return [
        `--- ${path}`,
        `+++ ${path} (dry-run)`,
...
function setValue(changes, agent, configPath, path, from, to, reason) {
    if (from === to)
        return;
    changes.push({
        agent,
...
function hardenPermission(profile, cfg, next, configPath, changes) {
    ...
    if (perm === "allow") {
        ...
        setValue(changes, "opencode", configPath, "permission", "allow", next.permission, "顶层 permission=allow 会放大所有工具权限，dry-run 建议改为显式分项配置。");
        return;
    }
    if (nextPerm.bash === "allow") {
        nextPerm.bash = "ask";
        setValue(changes, "opencode", configPath, "permission.bash", "allow", "ask", "Bash 无确认执行风险高，建议至少改为 ask。");
    }
    if (profile === "safe") {
        for (const key of ["edit", "webfetch"]) {
            if (nextPerm[key] === "allow") {
                nextPerm[key] = "ask";
                setValue(changes, "opencode", configPath, `permission.${key}`, "allow", "ask", "safe 基线要求高影响工具默认询问。");
            }
        }
    }
    if (Object.keys(nextPerm).length > 0)
        next.permission = nextPerm;
}
function hardenOpenCodeConfig(profile, configPath, cfg) {
    ...
    if (cfg.share === "auto") {
        ...
        next.share = target;
        setValue(changes, "opencode", configPath, "share", "auto", target, "share=auto 会自动上传会话上下文，建议改为手动或关闭。");
    }
    if (cfg.autoupdate === true) {
        next.autoupdate = false;
        setValue(changes, "opencode", configPath, "autoupdate", true, false, "受管环境建议关闭自动更新，改为审阅后升级。");
    }
    if (changes.length === 0)
        return undefined;
    return {
...
function hardenClaudeConfig(_profile, configPath, cfg) {
    const next = cloneJson(cfg);
    const changes = [];
    ...
    if (perm.defaultMode === "bypassPermissions") {
        ...
        setValue(changes, "claude-code", configPath, "permissions.defaultMode", "bypassPermissions", "default", "bypassPermissions 会无条件跳过所有工具确认，建议改回 default。");
    }
    // —— permissions.allow 中的无约束 Bash / 通配规则 → 移除 ——
    const allow = Array.isArray(perm.allow)
        ? perm.allow.filter((x) => typeof x === "string")
        : undefined;
    if (allow) {
        ...
        if (dangerous.length > 0) {
            ...
            perm.allow = kept;
            next.permissions = perm;
            setValue(changes, "claude-code", configPath, "permissions.allow", allow, kept, `移除 ${dangerous.length} 条无约束 Bash / 通配 allow 规则（等价放行任意命令）。`);
        }
    }
    // —— enableAllProjectMcpServers → false ——
    if (next.enableAllProjectMcpServers === true) {
        next.enableAllProjectMcpServers = false;
        setValue(changes, "claude-code", configPath, "enableAllProjectMcpServers", true, false, "自动启用项目内全部 MCP 可能加载不可信工具，建议改为按需批准。");
    }
    if (changes.length === 0)
        return undefined;
    return {
...
function hardenGeminiConfig(_profile, configPath, cfg) {
    const next = cloneJson(cfg);
    const changes = [];
    ...
    for (const [name, raw] of Object.entries(servers)) {
        ...
        if (s.trust === true) {
            s.trust = false;
            servers[name] = s;
            setValue(changes, "gemini", configPath, `mcpServers.${name}.trust`, true, false, `MCP "${name}" 的 trust=true 会绕过该 server 全部工具确认（等价 per-server YOLO）。`);
        }
    }
    if (changes.length > 0)
 ...
 * 明文 secret 不动（继续只报告）。
 */
function hardenOpenClawConfig(_profile, configPath, cfg) {
    const next = cloneJson(cfg);
    const changes = [];
    ...
    if (typeof gw.bind === "string" && !isLoopbackBind(gw.bind)) {
        ...
        gw.bind = "127.0.0.1";
        next.gateway = gw;
        setValue(changes, "openclaw", configPath, "gateway.bind", from, "127.0.0.1", "网关绑定非 loopback 地址会把 HTTP API 暴露到同网段/公网，收敛回 127.0.0.1。");
    }
    // —— gateway.tailscale.mode funnel/public/expose → off ——
    ...
    if (ts && typeof ts === "object" && !Array.isArray(ts)) {
        ...
        if (mode === "funnel" || mode === "public" || mode === "expose") {
            ...
            gw.tailscale = tsr;
            next.gateway = gw;
            setValue(changes, "openclaw", configPath, "gateway.tailscale.mode", from, "off", "Tailscale Funnel/public 会把网关发布到公网，收敛为 off 仅保留 Tailnet 内访问。");
        }
    }
    if (changes.length === 0)
        return undefined;
...
function hardenFile(label, configPath, harden, edits, warnings) {
    try {
        const source = readFileSync(configPath, "utf8");
        const cfg = asRecord(JSON.parse(source));
        const edit = harden(cfg);
        if (edit)
            edits.push({ ...edit, sourceHash: contentHash(source) });
    }
    catch (err) {
        warnings.push(`${label} 配置无法解析，已跳过：${err instanceof Error ? err.message : String(err)}`);
    }
}
export async function buildBaselineEdits(profile, ctx = buildContext()) {
    const warnings = [];
    const edits = [];
    // —— OpenCode ——
    const oc = await opencodeAdapter.discover(ctx);
    if (oc.configFound && oc.configPath) {
        hardenFile("OpenCode", oc.configPath, (cfg) => hardenOpenCodeConfig(profile, oc.configPath, cfg), edits, warnings);
    }
    else {
    ...
    if (claude.configFound && claude.configPath) {
        for (const file of ["settings.json", "settings.local.json"]) {
            ...
            if (!fileExists(p))
                continue;
            hardenFile("Claude Code", p, (cfg) => hardenClaudeConfig(profile, p, cfg), edits, warnings);
        }
    }
    ...
    if (gemini.configFound && gemini.configPath) {
        hardenFile("Gemini CLI", gemini.configPath, (cfg) => hardenGeminiConfig(profile, gemini.configPath, cfg), edits, warnings);
    }
    ...
    if (openclaw.configFound && openclaw.configPath) {
        hardenFile("OpenClaw", openclaw.configPath, (cfg) => hardenOpenClawConfig(profile, openclaw.configPath, cfg), edits, warnings);
    }
    return { edits, warnings };
}
export async function buildBaselinePlan(profile, ctx = buildContext()) {
    const { edits, warnings } = await buildBaselineEdits(profile, ctx);
    return {
        profile,
        dryRun: true,
        files: edits.map(publicPlan),
        warnings: warnings.map((w) => w === "未发现 OpenCode 配置，当前 baseline 无可处理文件。"
