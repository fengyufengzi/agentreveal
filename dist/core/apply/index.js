...
export async function applyBaseline(profile, ctx = buildContext(), options = {}) {
    ...
    for (const edit of edits) {
        if (contentHash(edit.configPath) !== edit.sourceHash) {
            throw new Error(`配置在 dry-run 计划生成后发生变化，已停止应用：${edit.configPath}`);
        }
    }
    const backup = createBackup(ctx.cwd, edits.map((edit) => ({ agent: edit.agent, path: edit.configPath })), `baseline-${profile}`);
    try {
        for (const edit of edits) {
        ...
        try {
            ...
            throw new Error(`应用失败，自动回滚也失败；请保留备份并人工恢复。应用错误：${reason}；回滚错误：${rollbackReason}`);
        }
...
export async function backupOpenCodeConfig(ctx = buildContext()) {
    ...
    const backup = createBackup(ctx.cwd, [{ agent: "opencode", path: found.configPath }], "manual-opencode");
    return { backupId: backup.id, files: backup.files.length, warnings: [] };
}
export function restoreLatestBaselineBackup(cwd) {
...
export function restoreBaselineBackup(cwd, id) {
