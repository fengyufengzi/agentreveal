...
export function assertClaudePlaintextTask(task, taskId) {
    assertTaskId(taskId);
    if (!task ||
        task.taskId !== taskId ||
        !task.requirements.some((requirement) => requirement.ruleId === "CLAUDE_PLAINTEXT_TOKEN")) {
        throw new Error("当前扫描中已找不到可备份的 Claude 明文凭证任务，请先重新扫描。");
    }
}
function credentialTaskId(manifest) {
...
function validatedCredentialBackup(cwd, backupId, configDir) {
    const manifest = readBackup(cwd, backupId);
    const taskId = credentialTaskId(manifest);
    const expectedPaths = new Set([...CLAUDE_SETTINGS_NAMES].map((name) => resolve(configDir, name)));
    ...
    if (manifest.files.length < 1 ||
        ...
        manifest.files.some((file, index) => file.agent !== "claude-code" ||
...
export function createClaudeCredentialBackup(input) {
    ...
    const backup = createBackup(input.cwd, configPaths.map((path) => ({ agent: "claude-code", path })), `${CLAUDE_CREDENTIAL_LABEL_PREFIX}${input.taskId}`);
    return {
        backupId: backup.id,
        taskId: input.taskId,
...
export function previewClaudeCredentialRestore(input) {
    ...
    return {
        ...
        changedFiles: state.filter((file, index) => file.sha256 !== manifest.files[index]?.sha256).length,
...
export function restoreClaudeCredentialBackup(input) {
    ...
    const { manifest, taskId } = validatedCredentialBackup(input.cwd, input.backupId, input.configDir);
    const state = backupRestoreFileState(manifest);
    if (restoreFingerprint(input.backupId, taskId, state) !==
