...
function pathUsesCaseInsensitiveLookup(path) {
...
function acceptanceKey(scopeId, taskId) {
...
function validateRuleSummaries(value, expectedRuleIds) {
    if (value === undefined)
        return undefined;
...
function validateTaskSummary(value, expectedTaskId) {
...
function validateRecordBase(value, expectedTaskId) {
    if (!value || typeof value !== "object" || Array.isArray(value)) {
...
function validateScopedRecord(value, expectedTaskId, expectedScopeId) {
...
function validateLegacyCollection(value) {
    if (!value || typeof value !== "object" || Array.isArray(value)) {
    ...
    for (const [taskId, history] of Object.entries(value)) {
        ...
        if (!Array.isArray(history))
            throw new Error(`接受记录历史无效：${taskId}`);
...
function validateDocument(value) {
    ...
    for (const [key, history] of Object.entries(raw.acceptances)) {
        ...
        acceptances[key] = history.map((record) => validateScopedRecord(record, first.taskId, first.scopeId));
    }
...
function taskSummary(task) {
    const uniqueSorted = (values) => [...new Set(values.map((value) => value.trim()).filter(Boolean))].sort((a, b) => a.localeCompare(b));
    return {
        taskId: task.taskId,
        family: task.family,
        source: task.source,
...
function normalizeExpiresAt(value, now) {
    ...
    if (!Number.isFinite(parsed.getTime()))
        throw new Error("expiresAt 不是有效日期。");
    if (parsed.getTime() <= now.getTime()) {
        throw new Error("expiresAt 必须晚于当前时间。");
    }
    return parsed.toISOString();
}
function statusAt(record, now) {
    if (record.revokedAt)
        return "revoked";
...
export class AcceptanceStore {
    ...
    read() {
        ...
        try {
            ...
            throw new Error(`无法读取接受记录 ${this.path}：${error instanceof Error ? error.message : String(error)}`);
        }
    ...
    list(options = {}) {
        const now = this.currentDate();
        const document = this.read();
        const scoped = Object.values(document.acceptances).flatMap((history) => history
            .filter((record) => options.allScopes || record.scopeId === this.scopeId)
            .map((record) => ({ ...record, status: statusAt(record, now) })));
        const legacy = options.includeLegacy
            ? Object.values(document.legacyAcceptances).flatMap((history) => history.map((record) => ({ ...record, status: "legacy" })))
            : [];
    ...
    isAccepted(taskId) {
        ...
        return (this.read().acceptances[key] ?? []).some((record) => statusAt(record, now) === "active");
    }
    ...
    revoke(taskId) {
        ...
        if (!record)
            throw new Error(`任务 ${taskId} 在当前项目没有可撤销的接受记录。`);
        const now = this.currentDate();
