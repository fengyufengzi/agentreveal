...
import { applyRuleIgnores } from "../triage/index.js";
...
function evidenceRows(evidence) {
    return Object.entries(evidence)
        ...
        return `<div class="ev"><span class="ev-k">${escapeHtml(k)}</span><span class="ev-v">${escapeHtml(val)}</span></div>`;
    })
        .join("");
}
...
function actionList(title, values, className = "") {
    if (values.length === 0)
        return "";
    return `<div class="action-list ${className}"><strong>${escapeHtml(title)}</strong><ol>${values
...
function postureSection(posture) {
    ...
    const cards = posture.agents
        ...
        const sources = state.configSources
            .map((source) => `<li><strong>${escapeHtml(source.kind)}/${escapeHtml(source.scope)}</strong> <span>${escapeHtml(source.status)}</span>${source.path ? ` <code>${escapeHtml(source.path)}</code>` : ""}<br><small>${escapeHtml(source.fields.join("、") || "无可识别字段")}</small></li>`)
            .join("");
        const permissions = state.permissions
            .map((permission) => `<li><code>${escapeHtml(permission.capability)}</code> ${escapeHtml(permission.decision)} · ${escapeHtml(permission.scope)}</li>`)
            .join("");
        ...
        const plans = remediationPlans
            .map((plan) => `<details class="posture-plan"><summary>${escapeHtml(plan.title)} · ${escapeHtml(plan.status)}</summary>
<p><strong>当前：</strong>${escapeHtml(plan.currentExplanation)}</p>
<p><strong>目标：</strong>${escapeHtml(plan.targetState)}</p>
<ol>${plan.steps
            .map((step) => `<li><strong>${escapeHtml(step.title)}</strong>：${escapeHtml(step.detail)}</li>`)
            .join("")}</ol>
<p><strong>为什么不自动执行：</strong>${escapeHtml(plan.automation.reason)}</p>
<ul>${plan.constraints
...
<div class="posture-head"><h3>${escapeHtml(state.displayName)}</h3><span class="posture-confidence">${escapeHtml(state.confidence)}</span></div>
<dl class="posture-facts">
<div><dt>Provider / 模型</dt><dd>${escapeHtml(state.route.providerClass ?? "未确认")} / ${escapeHtml(state.route.model ?? "未确认")}</dd></div>
<div><dt>请求链路</dt><dd>${escapeHtml(state.route.effectiveEndpoint ?? "未确认")} · ${escapeHtml(state.route.proxyKind)}${state.route.realUpstream
            ? ` → ${escapeHtml(state.route.realUpstream)}`
            : ""}</dd></div>
<div><dt>认证来源</dt><dd>${escapeHtml(state.auth.method)} · ${escapeHtml(state.auth.status)}${state.auth.sourceKind ? ` · ${escapeHtml(state.auth.sourceKind)}` : ""}</dd></div>
</dl>
...
function driftSection(drift) {
    if (!drift)
        return "";
    const status = drift.status === "no-baseline"
        ? "尚未保存可信状态"
    ...
    const events = drift.events
        .map((entry) => `<li><div><span class="priority pri-${escapeHtml(entry.priority.toLowerCase())}">${escapeHtml(entry.priority)}</span> <strong>${escapeHtml(entry.currentSummary)}</strong></div><p>${escapeHtml(entry.agentId)} · ${escapeHtml(entry.kind)} · ${escapeHtml(entry.change)}</p>${actionList("建议处理", entry.action)}${actionList("如何验证", entry.verification, "verify")}</li>`)
        .join("");
...
function relatedFindings(task) {
    return `<div class="related-findings"><strong>关联发现（${task.items.length}）</strong><ul>${task.items
        .map((item) => `<li><span class="related-agent">${escapeHtml(item.displayName)}</span><span>${escapeHtml(item.finding.title)}</span><code>${escapeHtml(item.finding.id)}</code></li>`)
        .join("")}</ul></div>`;
}
function requirementCard(requirement) {
    const acceptWhen = requirement.acceptWhen
        ? `<p class="requirement-accept"><strong>接受条件</strong> ${escapeHtml(requirement.acceptWhen)}</p>`
        : requirement.disposition === "observe"
            ? ""
            : `<p class="requirement-blocked"><strong>不可接受</strong> 当前规则没有已定义的安全接受条件。</p>`;
    return `<div class="rule-requirement"><div class="requirement-head"><code>${escapeHtml(requirement.ruleId)}</code><span class="priority pri-${requirement.priority.toLowerCase()}">${escapeHtml(requirement.priority)}</span><span>${escapeHtml(SEVERITY_LABEL[requirement.severity])}</span><span>${escapeHtml(fixModeLabel(requirement.fixMode))}</span></div><p><strong>原因</strong> ${escapeHtml(requirement.rationale)}</p>${actionList("下一步", requirement.nextSteps)}${actionList("如何验证", requirement.verification, "verify")}${acceptWhen}</div>`;
}
function requirementSection(task) {
    return `<div class="requirements"><h3>全部规则处置条件（${task.requirements.length}）</h3>${task.requirements.map(requirementCard).join("")}</div>`;
}
function remediationOutcome(task, profile = "balanced") {
    const actionable = task.requirements.filter((requirement) => requirement.disposition !== "observe");
    if (actionable.length === 0 ||
        actionable.some((requirement) => requirement.fixMode !== "baseline" ||
            !requirement.baselineProfiles?.[profile])) {
        return "assist";
    }
    return actionable.every((requirement) => requirement.baselineProfiles?.[profile] === "resolve")
        ? "complete"
        : "mitigate";
}
function taskFixMode(task) {
    const modes = new Set(task.requirements.map((requirement) => requirement.fixMode));
    if (modes.size === 1)
        return fixModeLabel(task.requirements[0].fixMode);
    return "组合处置（必须完成全部规则条件）";
}
function taskBaselineEffects(task) {
...
function remediationCommands(task) {
    const guide = buildRemediationGuide(task);
    const hasUsefulCommand = guide.commands.some((command) => command.kind !== "verify");
    if (!hasUsefulCommand)
        return "";
    const title = guide.mode === "baseline"
        ? `本机自动整改（${platformLabel(guide.platform)}，先预览）`
        : `本机安全引导（${platformLabel(guide.platform)}）`;
    const outcome = remediationOutcome(task);
    const commandOutcome = (item) => {
        if (item.kind === "verify")
            return "验证步骤";
        if (!item.completesRemediation)
            return "辅助步骤";
        if (outcome === "complete")
            return "完整解决";
        if (outcome === "mitigate")
    ...
    return `<div class="baseline-box remediation-box"><strong>${escapeHtml(title)}</strong>${guide.commands
        .map((item) => `<div class="command-row"><small>${escapeHtml(item.label)} <span class="command-outcome outcome-${commandOutcome(item) === "完整解决" ? "complete" : commandOutcome(item) === "风险缓解" ? "mitigate" : "assist"}">${escapeHtml(commandOutcome(item))}</span></small><code>${escapeHtml(item.command)}</code><button type="button" class="copy-command">复制命令</button></div>`)
        .join("")}${guide.notes
        .map((note) => `<small class="remediation-note">${escapeHtml(note)}</small>`)
        .join("")}</div>`;
}
function providerTrustCommand(task) {
...
function projectIgnoreCommands(task) {
    ...
    return `<div class="accept-box ignore-box"><strong>项目内不再提示这些已审核规则</strong>${candidates
        .map((candidate) => {
        const command = `agentguard ignore add ${task.taskId} --rule ${candidate.ruleId} ` +
            '--reason "填写审核依据；不要包含密钥或敏感信息"';
        return `<div class="command-row"><small>${escapeHtml(candidate.ruleId)}</small><code>${escapeHtml(command)}</code><button type="button" class="copy-command">复制命令</button></div>`;
    })
        .join("")}<small>只对当前项目和当前 Agent 生效，但 evidence/task ID 变化后仍会隐藏；P0/P1、强制修复和高风险家族不提供此操作。原因可能进入版本控制，请勿填写秘密。</small></div>`;
}
function actionCard(task, p0ExpiresOn) {
    ...
    const parts = [];
    parts.push(`<article class="action-card action-${disposition}" id="${escapeHtml(task.taskId)}">`);
    parts.push(`<div class="action-head"><span class="priority pri-${priority.toLowerCase()}">${priority}</span>` +
        `<span class="action-title">${escapeHtml(finding.title)}</span>` +
        (task.items.length > 1
            ? `<span class="task-size">${task.items.length} 项关联</span>`
            : "") +
        `</div>`);
    parts.push(`<div class="action-meta"><span>${escapeHtml(agents.join("、"))}</span>` +
    ...
    parts.push(`<p class="action-why"><strong>为什么要处理</strong> ${escapeHtml(task.requirements.length > 1
        ? `这是一个聚合任务，必须同时处理或逐项接受全部 ${task.requirements.length} 条关联规则；不能只按主规则判断。`
        : task.requirements[0].rationale)}</p>`);
    parts.push(requirementSection(task));
    parts.push(`<div class="fix-mode"><strong>处置方式</strong> ${escapeHtml(taskFixMode(task))}${taskBaselineEffects(task) ? ` · ${escapeHtml(taskBaselineEffects(task))}` : ""}</div>`);
    ...
    if (missingAcceptanceRules.length === 0) {
        parts.push(`<div class="accept-box"><strong>确认暂不修复整组任务</strong>` +
            `<code>agentguard risk accept ${escapeHtml(task.taskId)} --reason "填写真实接受原因"${priority === "P0" ? ` --expires ${escapeHtml(p0ExpiresOn)}` : ""} --confirm</code>` +
            `<small>命令执行前会再次列出全部规则、严重度和接受条件。接受后整组规则不再进入默认待办和退出码判断；可用 agentguard risk revoke ${escapeHtml(task.taskId)} 随时撤销。${priority === "P0" ? "P0 任务必须设置到期时间。" : ""}</small></div>`);
    }
    else {
        parts.push(`<div class="accept-box blocked"><strong>当前不能接受整组任务</strong><small>以下规则没有已定义的安全接受条件：${escapeHtml(missingAcceptanceRules.join("、"))}。请先完成修复。</small></div>`);
    }
    parts.push(`<div class="verify-box"><strong>处置后验证当前任务</strong><code>agentguard risk verify ${escapeHtml(task.taskId)}</code><small>本报告是静态快照；执行处置命令不会自动刷新。请运行 verify，必要时重新生成 HTML。</small></div>`);
    parts.push("</article>");
    return parts.join("");
}
function acceptedSection(tasks, acceptances) {
    if (tasks.length === 0)
        return "";
    return `<section class="accepted-section" id="actions-accepted"><div class="section-head"><div><h2>已接受风险</h2><p>这些事项已由用户确认暂不处理，不进入默认待办；技术证据仍保留在报告下方。</p></div><span class="section-count">${tasks.length}</span></div>${tasks
        ...
        const expires = record?.expiresAt
            ...
            : " · 长期有效";
        return `<article class="accepted-card" id="accepted-${escapeHtml(task.taskId)}"><div class="action-head"><span class="accepted-badge">已接受</span><span class="action-title">${escapeHtml(task.primary.finding.title)}</span></div><div class="action-meta"><span>${escapeHtml(taskAgents(task).join("、"))}</span><span class="rule-id">${escapeHtml(task.taskId)}</span></div><p><strong>接受原因</strong> ${escapeHtml(record?.reason ?? "未记录")}${expires}</p><code>agentguard risk revoke ${escapeHtml(task.taskId)}</code></article>`;
    })
        .join("")}</section>`;
}
function ignoredSection(policies, findings) {
    ...
    return `<section class="accepted-section ignored-section" id="actions-ignored"><div class="section-head"><div><h2>项目已忽略规则</h2><p>这些低优先级规则仍保留审计和技术证据，可随时撤销；策略会跨 evidence/task ID 变化持续生效。当前隐藏 ${findings.length} 项发现。</p></div><span class="section-count">${policies.length}</span></div>${policies
        .map((policy) => {
        const items = findings.filter((finding) => finding.agent === policy.agent && finding.finding.id === policy.ruleId);
        ...
        return `<article class="accepted-card ignored-card"><div class="action-head"><span class="accepted-badge">已忽略</span><span class="action-title">${escapeHtml(policy.ruleId)}</span></div><div class="action-meta"><span>${escapeHtml(items[0]?.displayName ?? policy.agent)}</span><span>${items.length} 项当前发现</span></div><p><strong>审核原因</strong> ${escapeHtml(policy.reason)}${expires}</p><p><strong>关联发现</strong> ${escapeHtml(titles.join("、") || "当前扫描未命中（策略仍有效）")}</p><div class="command-row"><code>${escapeHtml(command)}</code><button type="button" class="copy-command">复制命令</button></div></article>`;
    })
        .join("")}</section>`;
}
function actionSummary(tasks) {
    const counts = Object.fromEntries(DISPOSITION_ORDER.map((kind) => [kind, 0]));
    for (const task of tasks) {
        counts[normalizedDisposition(task.disposition)]++;
    }
    return `<nav class="action-summary" aria-label="行动摘要">${DISPOSITION_ORDER.map((kind) => `<a class="action-count count-${kind}" href="#actions-${kind}">` +
        `<strong>${counts[kind]}</strong><span>${DISPOSITION_LABEL[kind]}</span></a>`).join("")}</nav>`;
}
function topActions(tasks) {
    const top = tasks
    ...
    return `<section class="top-actions"><h2>建议先完成的 ${top.length} 项</h2><ol>${top
        ...
        const priority = normalizedPriority(task.priority);
        const agents = taskAgents(task);
        return `<li><a href="#${escapeHtml(task.taskId)}"><span class="priority pri-${priority.toLowerCase()}">${priority}</span>` +
            `<span><strong>${escapeHtml(task.primary.finding.title)}</strong><small>${escapeHtml(agents.join("、"))} · ${escapeHtml(DISPOSITION_LABEL[normalizedDisposition(task.disposition)])}${task.items.length > 1 ? ` · ${task.items.length} 项关联` : ""}</small></span></a></li>`;
    })
        .join("")}</ol></section>`;
}
function actionSections(tasks, p0ExpiresOn) {
    return DISPOSITION_ORDER.map((kind) => {
        const selected = tasks.filter((task) => normalizedDisposition(task.disposition) === kind);
        const cards = selected.length > 0
...
function findingCard(f) {
    ...
    parts.push(`<div class="f-head"><span class="badge sev-${f.severity}">${SEVERITY_LABEL[f.severity]}</span><span class="f-title">${escapeHtml(f.title)}</span><span class="f-id">${escapeHtml(f.id)}</span></div>`);
    if (f.description)
        parts.push(`<p class="f-desc">${escapeHtml(f.description)}</p>`);
    if (f.evidence)
        parts.push(`<div class="f-ev">${evidenceRows(f.evidence)}</div>`);
    if (f.recommendation)
        parts.push(`<p class="f-rec"><strong>建议</strong> ${escapeHtml(f.recommendation)}</p>`);
    if (f.remediation?.length)
        parts.push(`<div class="f-steps-wrap"><strong>手动整改步骤</strong><ol class="f-steps">${f.remediation
...
function agentOverview(report) {
    ...
    for (const r of report.results) {
        ...
        const cells = SEVERITY_COLUMNS.map((s) => `<td class="num${counts[s] ? "" : " zero"}">${counts[s]}</td>`).join("");
        rows.push(`<tr><td class="ag"><a href="#agent-${escapeHtml(r.agent)}">${escapeHtml(r.displayName)}</a></td><td>${topBadge}</td>${cells}<td class="num total">${r.findings.length}</td></tr>`);
    }
    ...
    if (correlations.length > 0) {
        ...
        const topBadge = top
            ...
            : "";
        const cells = SEVERITY_COLUMNS.map((s) => `<td class="num${counts[s] ? "" : " zero"}">${counts[s]}</td>`).join("");
        rows.push(`<tr><td class="ag"><a href="#correlation">跨 Agent 关联</a></td><td>${topBadge}</td>${cells}<td class="num total">${correlations.length}</td></tr>`);
    }
    if (rows.length === 0)
        return "";
    const head = `<tr><th>Agent</th><th>最高</th>${SEVERITY_COLUMNS.map((s) => `<th>${SEVERITY_LABEL[s]}</th>`).join("")}<th>合计</th></tr>`;
    return `<table class="overview"><thead>${head}</thead><tbody>${rows.join("")}</tbody></table>`;
}
function agentSection(r) {
    const parts = [];
    ...
    parts.push(`<h2>${escapeHtml(r.displayName)} ${status}</h2>`);
    if (r.discovery.configPath) {
        parts.push(`<p class="cfgpath">${escapeHtml(r.discovery.configPath)}</p>`);
    }
    const sorted = [...r.findings].sort((a, b) => SEVERITY_ORDER[a.severity] - SEVERITY_ORDER[b.severity]);
    for (const f of sorted)
        parts.push(findingCard(f));
    parts.push(`</section>`);
    return parts.join("");
}
/** 跨 Agent 关联区块（无关联项则为空字符串）。 */
function correlationSection(report) {
    const correlations = report.correlations ?? [];
    if (correlations.length === 0)
        return "";
    const sorted = [...correlations].sort((a, b) => SEVERITY_ORDER[a.severity] - SEVERITY_ORDER[b.severity]);
    const parts = [];
    parts.push(`<section class="agent" id="correlation">`);
    parts.push(`<h2>跨 Agent 关联 <span class="tag">${correlations.length} 项</span></h2>`);
    parts.push(`<p class="cfgpath">派生自各 Agent 端点/代理，标出多个 Agent 汇聚的单点失陷面。</p>`);
    for (const f of sorted)
        parts.push(findingCard(f));
    parts.push(`</section>`);
    return parts.join("");
}
function summaryBadges(report) {
    const counts = {
        critical: 0,
        high: 0,
        medium: 0,
    ...
    const badges = SEVERITY_COLUMNS.map((s) => `<button class="sum badge filter sev-${s}" data-filter="${s}">${SEVERITY_LABEL[s]} ${counts[s]}</button>`).join("");
    return all + badges;
}
const STYLE = `
...
`;
/** 生成完整 HTML 报告字符串。 */
export function renderHtmlReport(report, opts = {}) {
    const generatedAt = opts.generatedAt ?? new Date();
    const when = generatedAt.toISOString();
    const p0ExpiresOn = new Date(generatedAt.getTime() + 30 * 24 * 60 * 60 * 1000)
        .toISOString()
        .slice(0, 10);
    ...
    const acceptances = new Map((opts.acceptances ?? []).map((record) => [record.taskId, record]));
    ...
    const actionable = activeTasks.filter((task) => normalizedDisposition(task.disposition) !== "observe").length;
