...
import { cardGuidance, cardLabel, previousVsCurrent, sortByCardPriority, } from "../posture/drift-explain.js";
...
function projectIgnoreCommands(task) {
    ...
    return `<div class="accept-box ignore-box"><strong>项目内不再提示这些已审核规则</strong>${candidates
        .map((candidate) => {
        const command = `agentreveal ignore add ${task.taskId} --rule ${candidate.ruleId} ` +
...
function actionCard(task, p0ExpiresOn) {
    ...
    if (missingAcceptanceRules.length === 0) {
        parts.push(`<div class="accept-box"><strong>确认暂不修复整组任务</strong>` +
            `<code>agentreveal risk accept ${escapeHtml(task.taskId)} --reason "填写真实接受原因"${priority === "P0" ? ` --expires ${escapeHtml(p0ExpiresOn)}` : ""} --confirm</code>` +
            `<small>命令执行前会再次列出全部规则、严重度和接受条件。接受后整组规则不再进入默认待办和退出码判断；可用 agentreveal risk revoke ${escapeHtml(task.taskId)} 随时撤销。${priority === "P0" ? "P0 任务必须设置到期时间。" : ""}</small></div>`);
    }
    else {
    ...
    parts.push(`<div class="verify-box"><strong>处置后验证当前任务</strong><code>agentreveal risk verify ${escapeHtml(task.taskId)}</code><small>本报告是静态快照；执行处置命令不会自动刷新。请运行 verify，必要时重新生成 HTML。</small></div>`);
    parts.push("</article>");
    return parts.join("");
}
function acceptedSection(tasks, acceptances) {
    ...
    return `<section class="accepted-section" id="actions-accepted"><div class="section-head"><div><h2>已接受风险</h2><p>这些事项已由用户确认暂不处理，不进入默认待办；技术证据仍保留在报告下方。</p></div><span class="section-count">${tasks.length}</span></div>${tasks
        ...
        const expires = record?.expiresAt
            ...
            : " · 长期有效";
        return `<article class="accepted-card" id="accepted-${escapeHtml(task.taskId)}"><div class="action-head"><span class="accepted-badge">已接受</span><span class="action-title">${escapeHtml(task.primary.finding.title)}</span></div><div class="action-meta"><span>${escapeHtml(taskAgents(task).join("、"))}</span><span class="rule-id">${escapeHtml(task.taskId)}</span></div><p><strong>接受原因</strong> ${escapeHtml(record?.reason ?? "未记录")}${expires}</p><code>agentreveal risk revoke ${escapeHtml(task.taskId)}</code></article>`;
    })
        .join("")}</section>`;
}
