...
export function formatDrift(comparison) {
    const lines = ["AgentReveal 自可信状态以来"];
    if (comparison.status === "unavailable") {
        lines.push("可信状态不可读取：本机状态文件、身份密钥或权限异常。", "当前风险扫描仍可继续，但不会把未知状态自动设为可信；请检查 ~/.agentreveal 后重试。");
        return lines.join("\n");
    }
    if (comparison.status === "no-baseline") {
        lines.push("尚未保存可信状态。首次扫描不会自动信任当前配置。", "审核完成后运行 agentreveal drift baseline --confirm。");
        return lines.join("\n");
    }
    lines.push(`当前变化 ${comparison.activeEventCount} · 已恢复 ${comparison.resolvedEventCount}` +
    ...
    for (const { event, card } of sorted) {
        lines.push(`- ${cardLabel(card.cls)} ▌${event.agentId}`, `  [${event.priority}/${event.severity}] ${shortTitle(event)}`, `  ${event.currentSummary}`);
        const prev = previousVsCurrent(event);
        if (prev)
            lines.push(`  ${prev}`);
        lines.push(`  解读：${cardGuidance(card.cls)}`);
        if (event.action[0])
            lines.push(`  下一步：${event.action[0]}`);
...
//# sourceMappingURL=drift-format.js.map