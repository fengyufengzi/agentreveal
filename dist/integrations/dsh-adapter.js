...
import { validateModelSafeScan, } from "../core/integration/model-safe-scan.js";
...
export const runDshProcess = (request) => new Promise((resolveResult) => {
...
function mapProcessFailure(result) {
...
export async function inspectForDsh(options = {}) {
    ...
    const scan = await processRunner(request(["integration", "scan", "--format", "model-json"]));
...
export function renderDshResult(result) {
    if (!result.ok)
        return { kind: "error", text: FAILURE_TEXT[result.reason] };
    const { report } = result;
    const headline = result.status === "needs-attention"
        ? `发现 ${report.summary.actionableTaskCount} 个需要处理或确认的本地安全任务。`
        : "本次本地只读检查未发现高风险任务。";
    const risks = report.topRisks.map((risk, index) => `${index + 1}. [${risk.priority}/${risk.severity}] ${AGENT_LABELS[risk.agent]} · ` +
        `${CATEGORY_LABELS[risk.category]} · ${risk.ruleIds.join(", ")}`);
    const omitted = report.summary.omittedActionableTaskCount > 0
...
export async function runDshAgentRevealCommand(input) {
    ...
    return renderDshResult(await inspectForDsh({ cwd: input.cwd, signal: input.signal }));
}
