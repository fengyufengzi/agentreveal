...
export function buildGeminiFindings(data, providerPolicy = {}) {
    const findings = [];
    // —— 1. .env 明文密钥 ——
    ...
    for (const s of data.mcpServers) {
        ...
        if (s.trust) {
            findings.push({
                ...
                description: "该 MCP server 的所有工具调用将绕过确认（等价 per-server YOLO），恶意或被劫持的工具可无提示执行。",
    ...
    if (data.authType) {
        findings.push({
            ...
            description: data.authType === "gemini-api-key"
