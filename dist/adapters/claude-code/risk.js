...
export function isDangerousAllow(rule) {
    const r = rule.trim();
    // Bash 无参数约束（等价放行任意命令）或显式通配。
    return /^Bash$/i.test(r) || /^Bash\(\s*\*\s*\)$/i.test(r) || r === "*";
}
export function buildClaudeCodeFindings(data, providerPolicy = {}) {
    const findings = [];
    // —— base_url 风险 ——
    if (data.baseUrl) {
        ...
        if (cls.type === "local") {
            ...
            const proxyManagedOnly = data.proxyManagedPlaceholderPresent && !data.authTokenPresent;
            findings.push({
        ...
        else if (cls.type === "openai_compatible_unknown" ||
            cls.type === "unknown" ||
