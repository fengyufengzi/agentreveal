...
export function buildCodexFindings(data, providerPolicy = {}) {
    const findings = [];
    ...
    for (const s of data.mcpServers) {
        ...
        if (s.url) {
            ...
            const risky = cls.type === "openai_compatible_unknown" ||
                cls.type === "unknown" ||
                cls.type === "relay_or_proxy" ||
    ...
    if (data.trustedProjects.length > 0) {
        findings.push({
            ...
            description: "trusted 项目内 Codex 可能免审批执行命令，受信范围越大自动执行面越广。",
