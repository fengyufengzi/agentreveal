export function buildOpenClawFindings(data) {
    const findings = [];
    // —— 1. 渠道明文 secret ——
    for (const ch of data.channels) {
        if (ch.hasAppSecret) {
            findings.push({
                ...
                description: "appSecret 直接写在 openclaw.json 的 channels 配置中，配置泄露或仓库推送意外提交即凭据泄露。",
                evidence: { channel: ch.channel, field: "appSecret" },
                recommendation: "改为通过环境变量注入（如 channels.feishu.appSecret = \"${FEISHU_APP_SECRET}\"），或迁移到独立密钥管理。",
                fixable: false,
            });
        }
        if (ch.hasToken) {
            findings.push({
                ...
                description: "渠道 token 直接写在 openclaw.json 中。推荐改用环境变量注入或临时轮换。",
    ...
    if (data.gateway?.auth?.token) {
        findings.push({
            ...
            description: "HTTP API 鉴权 token 明文写在 openclaw.json。任何能读到此配置的用户/进程都拥有完整 API 权限。",
    ...
    if (data.gateway?.bind && !isLoopbackBind(data.gateway.bind)) {
        findings.push({
            ...
            description: "网关监听非本地回环地址（如 0.0.0.0 或 LAN IP），意味着同网段或公网可访问。即使有鉴权，暴露面仍偏大。",
    ...
    if (data.gateway?.tailscale && typeof data.gateway.tailscale === "object") {
        ...
        if (mode === "funnel" || mode === "public" || mode === "expose") {
            ...
            findings.push({
                ...
                description: "Tailscale Funnel / public 会把网关发布到公网，请确认是否需要该暴露。",
                evidence: { mode },
    ...
    for (const [ws, ids] of wsMap) {
        if (ids.length > 1) {
            findings.push({
                ...
                description: "不同 agent 使用同一 workspace 路径，可能导致上下文/MEMORY/工具权限互相覆盖或泄露。",
    ...
    for (const p of data.plugins) {
        if (!p.source)
            continue;
        const suspicious = SUSPICIOUS_SOURCE_PATTERNS.some((re) => re.test(p.source)) &&
        ...
        if (suspicious) {
            findings.push({
                ...
                description: "插件 source 指向 file:// 或 http:// 本地/远端路径，未走 npm 包签名验证，存在供应链风险。",
