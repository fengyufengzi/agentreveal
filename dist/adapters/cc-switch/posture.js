...
function consumerAgentId(appType) {
...
function localEndpoint(address, port) {
...
export function buildCcSwitchPosture(data, dbPath, findings, providerPolicy = {}) {
    const routes = [];
    for (const proxy of data.proxies) {
        ...
        if (!consumer)
            continue;
        const current = data.providers.find((provider) => provider.appType === proxy.appType && provider.isCurrent);
        const realUpstream = current?.baseUrl ?? (current?.name ? `provider:${current.name}` : undefined);
        routes.push({
            ...
            ...(current?.baseUrl
                ? {
                    providerClass: classifyBaseUrl(current.baseUrl, providerPolicy).type,
                }
                : {}),
        });
    }
    const activeProviders = data.providers.filter((provider) => provider.isCurrent || provider.inFailoverQueue);
    const fields = [
        "schemaVersion",
        ...activeProviders.map((provider) => `providers.${fieldToken(provider.appType)}.${provider.isCurrent ? "current" : "failover"}`),
        ...data.proxies
