...
- Electron 桌面应用、可双击的本地开发启动器，以及完成 Developer ID 签名、Apple 公证与 staple 的
  Apple Silicon DMG；
- `0.0.5-pilot.3` npm CLI 与 macOS Desktop 已通过同一 GitHub Pre-release 联合公开。

因此，当前阶段正式定义为：

> **0.0.5 Public Preview 已联合公开，E2–E5 首轮开发已完成；当前按稳定里程碑冻结候选并定期联合公开。**

## 2. 当前开发主线

Effective Configuration 与 Drift 的首轮开发已经完成；后续迭代继续围绕以下主线收敛准确性、
解释性、可恢复性和真实使用体验，不再把外部用户访谈或 20 人 Pilot 作为开发前置门槛：
...
6. CLI 与 macOS Desktop 继续共用版本和 core；达到稳定里程碑后冻结候选，并执行 npm、签名公证 DMG
   和 GitHub Release 的联合发布；
7. 下载、重复使用、Issue 和团队请求改为发布后的方向信号，不阻断本阶段开发。

不得同时展开通用 MCP 动态扫描器、后台常驻监控、团队管理后台、企业多租户、Secret Vault、
SIEM / DLP / IAM 集成或运行时拦截。
...
迁移备份。AgentGuard 不代替目标工具真实鉴权，也不得伪装成完全一键修复。
...
2026-07-24 决定采用“持续开发、稳定里程碑联合发布”的节奏。单个 Milestone 的功能开发不再因 npm、
GitHub Release 或 Apple 公证尚未执行而保持未完成；准备发布版本时，才冻结同一提交并一次性完成 tarball、
...
`0.0.5-pilot.3` 已完成首次入口、风险处置语义和 CLI/Desktop 联合公开。`0.0.6-pilot.*` 已完成
Effective Configuration、Drift 与 H0–H7 安全整改事务的首轮功能开发；后续按稳定里程碑继续打磨并定期联合公开。
公开时仍保持两个入口共用 core、使用同一版本并通过完整联合发布门禁。若要改变此边界，必须先更新本文件并
记录决策原因，不能只在代码提交或聊天中改变方向。
