# AgentReveal CLI Pilot 试用说明

> 预计用时：15–20 分钟。请在你日常使用 AI Coding Agent 的项目目录中执行。

> 本文件对应 Public Preview 版本。CLI 可从 npm `next` 安装，也可使用 GitHub Release 的固定 tarball。
...
`agentreveal-0.0.7-pilot.2.tgz`，然后执行：
...
| 命令 | 作用 | 是否修改配置 |
|---|---|---|
| `agentreveal doctor` | 找出本机已配置的 Agent 和配置路径 | 否 |
| `agentreveal scan` | 扫描 Provider、代理、MCP、明文密钥和危险权限等风险 | 否 |
| `agentreveal map` | 展示多个 Agent、代理和真实上游之间的关系 | 否 |
| `agentreveal report --format html` | 在当前目录生成便于本地查看的 `agentreveal-report.html` | 否 |
...
AgentReveal 会拒绝恢复，避免覆盖新内容。备份包含完整原配置，请勿复制或上传备份目录。

### 确认某项风险暂时不处理

> 以下命令属于当前 Public Preview 版本。风险接受只影响当前项目和本机，不会自动同步到团队成员。

如果报告中的 MCP、权限或其它配置已经由你确认暂时不处理，可以复制报告卡片里的 `task-...`，记录接受原因：

```bash
agentreveal risk accept task-xxxxxxxxxxxx --reason "个人自建服务，已核对 TLS、访问控制和数据范围"
```

上面的命令只显示整组任务的全部规则、严重度和接受条件，不会写入。确认这些条件全部成立后执行：

```bash
agentreveal risk accept task-xxxxxxxxxxxx --reason "个人自建服务，已核对 TLS、访问控制和数据范围" --confirm
agentreveal report --format html
```
...
```bash
agentreveal risk accept task-xxxxxxxxxxxx --reason "限时测试" --expires 2026-12-31 --confirm
```
...
```bash
agentreveal trust add "relay.example.com" --kind trusted --reason "本人维护，已核对 TLS、访问控制和数据范围"
...
```bash
agentreveal trust remove "relay.example.com" --kind trusted --reason "服务已停用或需要重新审核"
```

### 忽略当前项目中已审核的低优先级规则

如果报告明确给出了 `agentreveal ignore add ...`，且你希望同一 Agent 的该条规则在 evidence 或 task ID
变化后也不再重复提示，可以直接复制命令：
...
生效。它也与 `trust add` 不同：trust 只确认 Provider 端点归属。AgentReveal 不允许项目级忽略 P0/P1、
...
只反馈某条规则是否准确及处置结果时，使用 [`rule-feedback.md`](rule-feedback.md) 的最小命令和专用 Issue
表单；该路径不需要 task ID、环境信息、诊断或自由文本。
...
默认卸载只删除命令，不删除 `~/.agentreveal` 的本地审计记录、项目 `.agentreveal` 的整改备份、项目 Provider
