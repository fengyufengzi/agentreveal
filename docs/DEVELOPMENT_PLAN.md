...
- [ ] 完成公开仓库内置隐私历史清理；第三方密钥历史复核已通过，最终资产仍待复核；
...
1. **确认发布身份与展示**：Demo 视频、封面和生成脚本已就绪；仍需确认最终 Public Preview 版本号、npm
   公开包名和作者公开邮箱。registry 的 404 只表示检查时未占用，不构成保留。
2. **建立干净公开历史**：从已审核当前树同步到干净公开候选仓库，或经明确授权重写现有历史；随后要求
   `sanitize:history` 归零，并再次运行第三方扫描。当前开发仓库仍有 23 项历史隐私元数据命中。
